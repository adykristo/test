...
function corsFor(req:Request){
  ...
  return {"Access-Control-Allow-Origin":origin||allowed[0]||"null","Access-Control-Allow-Headers":"authorization, x-client-info, apikey, content-type","Access-Control-Allow-Methods":"POST, OPTIONS","Vary":"Origin"};
}
...
const cleanJson=(text:string)=>JSON.parse(text.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/,""));
...
function imageParts(images:unknown){
  ...
  return images.slice(0,8).flatMap((src)=>{
    ...
    total+=m[2].length;if(total>8000000)throw new Error("Total gambar terlalu besar");
    return [{inlineData:{mimeType:m[1],data:m[2]}}];
  });
}

function rules(){return `Anda adalah editor soal KlinikFisikapku dan ahli pendidikan Indonesia.
...
Deno.serve(async(req)=>{
  try{
    ...
    const body=JSON.parse(raw),action=String(body.action||""),p=body.payload||{};
    const key=Deno.env.get("GEMINI_API_KEY");if(!key)throw new Error("GEMINI_API_KEY belum dipasang pada Supabase Secrets.");
    const configuredModel=Deno.env.get("GEMINI_MODEL");if(!configuredModel)throw new Error("GEMINI_MODEL belum dipasang pada Supabase Secrets.");
    const model=configuredModel.replace(/^models\//,"");if(!/^[A-Za-z0-9._-]{3,100}$/.test(model))throw new Error("GEMINI_MODEL tidak valid.");
    let prompt="",assistant=false;
    if(action==="adminAsistenSoal"){
      assistant=true;const tugas={variasi:"Buat beberapa variasi bermakna dari soal/instruksi",solusi:"Susun solusi langkah demi langkah dan periksa hasil akhir",osn:"Analisis level OSN, konsep inti, jebakan, dan strategi penyelesaian",audit:"Audit konsep, data, satuan, bahasa, opsi, kunci, dan keadilan soal"}[p.jenis]||"Analisis soal";
    ...
    }else if(!["adminBuatSoalGemini","adminAnalisisWordGemini","adminRapikanSoalGemini"].includes(action))throw new Error("Aksi AI tidak dikenali.");
    else prompt=questionPrompt(action,p);
    const parts=[{text:prompt},...imageParts(p.images)],endpoint=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(key)}`;
    const generationConfig:any={temperature:assistant?.35:.25,maxOutputTokens:32768};if(!assistant)generationConfig.responseMimeType="application/json";
    const response=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{role:"user",parts}],generationConfig})});
    const raw=await response.json();if(!response.ok)throw new Error(raw?.error?.message||"Gemini gagal memproses permintaan.");
    const text=raw?.candidates?.[0]?.content?.parts?.map((x:any)=>x.text||"").join("")||"";if(!text)throw new Error("Gemini tidak mengembalikan hasil.");
    await admin.from("member_ai_usage").insert({admin_id:user.id,action,model,item_count:Number(p.jumlah)||0});
    if(assistant)return json(req,{ok:true,analysis:text.slice(0,200000)});
    const parsed=cleanJson(text),questions=Array.isArray(parsed)?parsed:(parsed.questions||[]);
    if(!Array.isArray(questions)||!questions.length||questions.length>40)throw new Error("Format atau jumlah hasil Gemini tidak valid.");
    for(const q of questions){if(!q||typeof q!=="object"||String(q.soal||"").length>30000||JSON.stringify(q).length>150000)throw new Error("Salah satu soal AI terlalu besar atau tidak valid.");}
  ...
  }catch(error:any){console.error("gemini-question-studio",error);try{return json(req,{ok:false,error:String(error?.message||error)},400);}catch{return new Response('{"ok":false,"error":"Permintaan ditolak"}',{status:403,headers:{"Content-Type":"application/json"}});}}
});
